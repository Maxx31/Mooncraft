...
Ray::Ray(glm::vec3 position, glm::vec3 direction, World& world, float reach = 10.0f) 
{
  ...
  while (!successful && planes[0].getHitDistance() <= reach) 
  {
    ...
    if (maybeBlockPosition.has_value() && World::isValidBlockPosition(maybeBlockPosition.value())) {
      ...
      BlockData blockToCheck = *block;
      if (blockToCheck.type != BlockData::BlockType::air) {
        successful = true;
        std::optional<glm::ivec3> maybeNeighbor =
           AxisPlane::rayHitsToBlockPosition(prevClosestHits[0], prevClosestHits[1]);

        hitTarget = {blockPosition, blockToCheck, maybeNeighbor.value(), hasNeighbor && maybeNeighbor.has_value()};
      }
    }
